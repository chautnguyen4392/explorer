...
function update_address(hash, blockheight, txid, amount, type, cb) {
  ...
  console.log("TACA database.js, update_address ===> update Address db for address = %s, addr_inc = %s", hash, util.inspect(addr_inc, false, null, true));
  ...
  Address.findOneAndUpdate({a_id: hash}, {
    ...
    if (err) {
      ...
      if ( hash != 'coinbase' ) {
        console.log("TACA database.js, update_address ===> update AddressTx db for address = %s, txid = %s, balance = %s", hash, txid, addr_inc.balance);
...
function save_tx(txid, blockheight, cb) {
  ...
  lib.get_rawtransaction(txid, function(tx){
    ...
    console.log("TACA database.js, save_tx ===> txid = %s, tx = %s", txid, util.inspect(tx, false, null, true));
      ...
      lib.prepare_vin(tx, function(vin) { // prepare list of {vin_address, sent_amount}
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) { // // prepare list of {vout_address, received_amount}
          lib.syncLoop(vin.length, function (loop) {
            ...
            console.log("TACA database.js, save_tx ===> update vin[%s/%s], address = %s, amount = %s", i, vin.length, nvin[i].addresses, nvin[i].amount);
            ...
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                ...
                console.log("TACA database.js, save_tx ===> update vout[%s/%s], address = %s, amount = %s", t, vout.length, vout[t].addresses, vout[t].amount);
              ...
              lib.calculate_total(vout, function(total){
                ...
                console.log("TACA database.js, save_tx ===> update tx db for txid = %s, total = %s, tx.time = %s, tx.blockhash = %s, blockheight = %s", tx.txid, total, tx.time, tx.blockhash, blockheight);
